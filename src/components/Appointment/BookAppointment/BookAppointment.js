...
const BookAppointment = ({date}) => {
    return (
        <div className="container">
            <h2 className="text-center brand-text">Available appointment on {date.toDateString()}</h2>
            <div className="row my-5">
                {
                 bookingData.map(booking => <BookingCard key={booking._id} date={date} booking={booking} date={date}></BookingCard>)
                }
